...
public class ExtractPatchExecutor extends AbstractExecutor{
	...
	public File getCodeFile() {
		return new File(getProjectDir()+File.separator+getGerExpRec());
	}

	@Override
	public File getBinaryFile() {
		
		return new File(getGerExpRec().replaceAll(regx, getProjectDir()+File.separator+"bin").replace(".java", ".class"));
	}
	...
	public File getTargetBinaryFile() {
		...
		String finalBinaryPath = getGerExpRec().replaceAll(regx,getTargetDir()+File.separator+getScope(scope)).replace(".java", ".class");
		File fatherDir = new File(finalBinaryPath.substring(0, finalBinaryPath.lastIndexOf(File.separator)));
		if(!fatherDir.exists()){
			fatherDir.mkdirs();
		}
	...
	public File getTargetCodeFile() {
		...
		m.find();
		String scope = m.group(3);
		String finalBinaryPath = getGerExpRec().replaceAll(regx,getTargetDir()+File.separator+getScope(scope));
		File fatherDir = new File(finalBinaryPath.substring(0, finalBinaryPath.lastIndexOf(File.separator)));
		if(!fatherDir.exists()){
			fatherDir.mkdirs();
		}
